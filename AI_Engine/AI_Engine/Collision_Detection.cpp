...
void Predator_Detection(ball * object)
{
	...
	else if (Conditional_Judge(object, '&', 157.0f, 200.0f, 55.0f, 200.0f))//第8區:Maze 右下方的區域
	{
		...
		if (Wall_Detection(object, 177.0f, 185.0f, 188.0f, 200.0f))//右下角cylinder
			...
			Reaction(object);
		else if (Conditional_Judge(object, '&', 162.0f, 178.0f, 149.0f, 163.0f))//第3區:右下的Portal
			DecideDirection(object);
	}
	else if (Conditional_Judge(object, '&', 0.0f, 43.0f, 0.0f, 157.0f))//第9區:Maze 左上方的區域
	{
		if (object->Area == 9)
		{
			object->BottleNeck++;
	...
	else if (Conditional_Judge(object, '&', 43.0f, 157.0f, 0.0f, 41.0f))//第10區:Maze 右下方的區域
	{
		if (object->Area == 10)
		{
			object->BottleNeck++;
